...
const UserManagement = () => {
    ...
    const handleToggleLock = async (userId, currentStatus) => {
        ...
        try {
            // Gọi API PATCH hoặc PUT để update trạng thái
            ...
            setUsers(users.map(user => 
                user.id_user === userId ? { ...user, is_locked: !currentStatus } : user
            ));
    ...
    const handleImportSubmit = async (e) => { 
        ...
        try {
            // 3. Gọi API
            // Axios tự động nhận diện FormData và set Header 'Content-Type': 'multipart/form-data'
            const res = await api.post('/admin/users/import', formData);

            // 4. Xử lý kết quả thành công
            // Backend trả về: { message, totalRows, inserted, skipped }
            setMessage({ 
                type: 'success', 
            ...
            // Reset file input
            setCsvFile(null);
            // Reset giá trị của thẻ input file trên DOM (để chọn lại file cũ được)
        ...
        <div className="bg-white p-6 rounded shadow-md">
                            ...
                            <tbody>
                                {users.map((user) => (
                                    <tr key={user.id_user} className="hover:bg-gray-50 transition">
                                        <td className="p-3 border text-sm text-gray-700">#{user.id_user}</td>
                                        ...
                                        <td className="p-3 border text-center space-x-2">
                                            {/* Nút Khóa / Mở khóa */}
                                            <button 
                                                onClick={() => handleToggleLock(user.id_user, user.is_locked)}
                                            ...
                                            {/* Nút Xóa (Nếu cần) */}
                                            {/* <button className="text-xs bg-red-100 text-red-600 px-3 py-1 rounded border border-red-200 hover:bg-red-200">Xóa</button> */}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
            ...
            {activeTab === 'create' && (
                <form onSubmit={handleCreateSubmit} className="max-w-lg mt-4">
                     {/* Copy lại form Create từ code trước */}
                     <div className="mb-4">
